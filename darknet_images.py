...
def check_batch_shape(images, batch_size):
    ...
    if len(set(shapes)) > 1:mask: 99%	(left_x:   43   top_y:   28   width:   40   height:   39)
mask: 100%	(left_x:  134   top_y:   24   width:   32   height:   34)
mask: 99%	(left_x:  205   top_y:   20   width:   35   height:   41)
...
def save_annotations(name, image, detections, class_names):
    ...
    with open(file_name, "w") as f:
        for label, confidence, bbox in detections:
            x, y, w, h = convert2relative(image, bbox)< /home/fadi/다운로드/Yolo_mark/x64/Release/data/train.txt >
...
def batch_detection_example():
    ...
    network, class_namebatch_detection_examples, class_colors = darknet.load_network(
        args.config_file,
        args.data_file,
        args.weights,
